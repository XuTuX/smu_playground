...
export function RankingExplorer({ scores }: { scores: ScoreRecord[] }) {
  ...
  return <><div className="ranking-filters"><label>게임<select value={gameId} onChange={(event) => setGameId(event.target.value)}><option value="">전체 게임</option>{games.map((game) => <option value={game.id} key={game.id}>{game.code.replace("GAME ", "게임 ")} · {game.name}</option>)}</select></label><label>학과<select value={departmentId} onChange={(event) => setDepartmentId(event.target.value)}><option value="">전체 학과</option>{departments.map((department) => <option value={department.id} key={department.id}>{department.name}</option>)}</select></label>{standings.length > 0 && <span className="filter-result-count">기록 {standings.length}개</span>}</div><PlayerRanking standings={standings} /></>;
}
