...
- `departments`: 학과 기준 정보. Supabase에는 대학 구분 없이 학과만 저장합니다.
...
`supabase/migrations/20260908065355_admin_student_scores.sql`이 초기 기준 데이터, RLS, 권한을 포함하며 이후 마이그레이션이 관리자 수정·삭제와 팀전·감사 로그를 확장합니다. 최신 마이그레이션은 기존 대학 테이블과 학과의 대학 연결을 제거하고 학과 데이터만 유지합니다. 파일명 순서대로 모두 적용해야 합니다.
