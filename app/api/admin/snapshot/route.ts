...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ summary: { playCount: dashboard.playCount, playerCount: dashboard.playerCount, champion: dashboard.champion.departmentName }, sync: snapshot.status }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Supabase 연결에 실패했습니다.";
    return Response.json({ summary: { playCount: 0, playerCount: 0, champion: "확인 필요" }, sync: { mode: "supabase", state: "error", totalRows: 0, error: message } }, { headers: { "Cache-Control": "no-store" } });
  }
}
